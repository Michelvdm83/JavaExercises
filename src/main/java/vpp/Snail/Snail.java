...
public class Snail {
    public static void main(String[] args) {
        int[][] snailMe = {{1, 2, 11, 11}, {5, 3, 4, 11}, {7, 8, 9, 12}, {13, 14, 15, 16}};
    ...
    private static int[][] snail(int[][] original) {
        ...
        int[] normalSnail = new int[side * side];
        ...
        for (int i : theWay) {
            int x = i / side;
            int y = i % side;
            normalSnail[count] = original[x][y];
            returnArray[x][y] = temp.get(count);
            count++;
        }
        System.out.println(Arrays.toString(normalSnail));
